...
Shader "Unlit/RangeProjectionWall"
{
    Properties
    {
        //[PerRendererData] _MainTex ("Texture", 2D) = "white" {}
        [PerRendererData] _FlareColor("lare Color", Color) = (1, 1, 1, 1)
        [PerRendererData] _LightColor("Light Color", Color) = (1, 1, 1, 1)
        [PerRendererData] _FlareHeight ("Flare Height", Float) = 0.2
        [PerRendererData] _LightHeight ("Light Height", Float) = 0.1
    }
    SubShader
    {
        Tags 
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float distance = length(i.worldPos.xyz - wpos.xyz);
                float flareStrength = clamp(abs(distance / _FlareHeight), 0, 1);
                float lightStrength = clamp(abs(distance / _LightHeight),0, 1);

                fixed4 color = _FlareColor * _FlareColor.a * lightStrength + _LightColor * (1 - lightStrength);
