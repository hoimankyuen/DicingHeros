Shader "SimpleMaskCutoff/HalfOntopImageCutoff"
{
	Properties
	{
		...
		_CutoffFrom("Cutoff From", Range(0, 1)) = 0
		_CutoffTo("Cutoff To", Range(0, 1)) = 0.5
	}
	SubShader
	{
		Tags
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_TARGET
			{
				...
				if (_CutoffTo > _CutoffFrom)
				{
					col.a *= step(cutoff.a, _CutoffTo) * step(_CutoffFrom, cutoff.a) * _OnTopAlpha;
				}
				else if (_CutoffTo < _CutoffFrom)
				{
					col.a *= (1 - (1 - step(cutoff.a, _CutoffTo)) * (1 - step(_CutoffFrom, cutoff.a))) * _OnTopAlpha;
				}
				else
				{
					col.a = 0;
